...
`react-native-geolocation` 라이브러리를 통해 전달받은 위도, 경도, 고도에 대한 데이터는 소수점 최대 15자리까지 전달받습니다.<br>
한국을 기준으로 위도, 경도 데이터를 미터 단위로 변환한 값은 다음과 같습니다.

- 위도 1도 = 약 111,000m
- 경도 1도 = 약 88,804m

그러나 내 주변에 있는 메모를 띄우는 것이기 때문에, 사용자와의 거리가 위도와 경도가 1도까지 차이 나는 경우는 AR 화면에서 띄울 수 없습니다.<br>
그렇기에 더 작은 단위로 변환한 값은 다음과 같습니다.
