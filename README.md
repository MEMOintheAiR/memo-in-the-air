...
"봄에 이곳에 오면 이 나무 앞에서 사진 찍으면 예쁠 것 같아"
<br>
<br>
이처럼 특정 장소와 연관된 해야 할 일이나 사소한 감상을 기록하고 싶을 때가 있지만, 그 생각을 장소에 저장할 방법은 많지 않습니다. 대부분의 메모 애플리케이션들은 시간과 텍스트 중심으로 설계되어 있어, 특정 장소와 연결된 메모를 시각적으로 남기는 데는 한계가 있었습니다. 그래서 "특정 공간에 가상의 메모를 띄우면 어떨까?"라는 생각을 하게 되었습니다.
